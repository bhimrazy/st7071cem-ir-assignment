"""Document storage: the source of truth a collection is rebuilt from.

The inverted index is *derived* data -- it can always be regenerated by
re-analyzing the documents. The documents cannot be regenerated from the
index, because analysis is lossy (stemming and stopword removal throw
information away). So the documents are what gets persisted.
"""
