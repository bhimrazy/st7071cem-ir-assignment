...
def arrow(ax, start, end, label="", offset=0.0):
    ...
    if label:
        ...
        ax.text(mx, my, label, ha="center", va="center", fontsize=8.2,
                color=MUTED, backgroundcolor="white")
...
def architecture() -> None:
    ...
    box(ax, 3.9, 6.1, 2.6, 0.9, "Polite crawler",
        "robots.txt, 5s delay", FILL_APP)
    box(ax, 7.4, 6.1, 2.2, 0.9, "Scheduler", "weekly re-crawl", FILL_APP)

    box(ax, 0.4, 4.5, 9.2, 1.0, "Collection  (documents + persistence)",
        "append-only log, compaction, background fsync", FILL_CORE)

    box(ax, 0.4, 2.9, 2.9, 1.0, "Analyser",
        "tokenise, stopwords, stem", FILL_CORE)
    box(ax, 3.6, 2.9, 2.9, 1.0, "Inverted index",
        "term to postings", FILL_CORE)
    box(ax, 6.8, 2.9, 2.8, 1.0, "Ranking",
        "TF-IDF / BM25", FILL_CORE)

    box(ax, 1.6, 1.3, 3.2, 0.95, "FastAPI", "/api/search", FILL_APP)
    box(ax, 5.4, 1.3, 3.2, 0.95, "React interface",
        "Scholar-style results", FILL_APP)
    ...
    ax.text(9.55, 5.0, "miniseek\nengine core", fontsize=8.4, color=MUTED,
            ha="right", va="center", style="italic")
    ax.text(0.42, 0.72, "Indexing flows downward; a query flows upward through "
                        "the same components.",
